 ...
 *
 * One can use the function provided to open and close all the log files at once.
 ...
 * Ability to specify the type of header using bit flags already defined in a macro.
 * Ability to mark the provided packet as incmoming or outgoing using a flag defined in a macro.
 */
 ...
 * Extracts the toggled bits from the 8-bit mask and calls the appropriate logging function accordingly.
 *
 * @param[in] header A pointer which points to the Header of any type.
 * @param[in] flags A mask whose different bits represent unique type of header passed.
